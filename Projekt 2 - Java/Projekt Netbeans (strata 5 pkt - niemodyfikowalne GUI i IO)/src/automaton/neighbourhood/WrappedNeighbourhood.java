 ...
 * @author shafe_000
 */
public class WrappedNeighbourhood implements NeighbourhoodStates{
    ...
    public int getNumberOfNeighbourhoodByState(Cells cells, int x, int y, int state) {
        ...
        if(cells.getCell(xm1,ym1).getValue()==state) number++;
        if(cells.getCell(xp1,yp1).getValue()==state) number++;
        if(cells.getCell(xm1,yp1).getValue()==state) number++;
        if(cells.getCell(xp1,ym1).getValue()==state) number++;
